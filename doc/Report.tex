...
\\
\indent The first major challenge that was discovered was in benchmarking the programs. Each assembler program ran extremely fast, and it seemed that all the stand ways of benchmarking through time.h returned '0' as the time taken. The solution to this was the use of the 'rdtsc' assembler code running inline with the code to be benchmarked. While this could be unreliable, due to the CPU halting process, essentially randomly, on average this is a good method. It returns CPU 'ticks' which have occured between two such commands.
\\
